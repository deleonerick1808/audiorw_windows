...
void audiorw::write(
  ...
  if (!(output_codec = const_cast<AVCodec *>(avcodec_find_encoder(codec_id)))) {
    cleanup(codec_context, format_context, resample_context, frame, packet);
    throw std::runtime_error(
  ...
  int sample = 0;
  std::vector<double> audio_data(audio.size() * codec_context->frame_size);
  while (true) {
    if (sample < (int) audio[0].size()) {
      ...
      const uint8_t * audio_data_ = reinterpret_cast<uint8_t *>(audio_data.data());
